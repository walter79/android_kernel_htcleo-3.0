...
/*
 * Battery driver Code modified by Droidzone.
 * Added:
 * 	Sibere's Battfix: http://forum.xda-developers.com/showthread.php?t=841580
 * 	Droidzone's Extended Batteryfix: http://forum.xda-developers.com/showthread.php?t=1334052
 * Last modified on Jan 24. Fixed and reverted Battfix code to Sibere's original values. Extended
 * battfix left in place.
 *
 */
	...
	* to 100)
	...
	if (htc_batt_info.rep.battery_full)
	    ...
	    (htc_batt_info.rep.level == 100)) {
	    	if (More_Charge_Extend_Time()) {
				htc_batt_info.rep.battery_full = 1;
				// Sibere's Battfix code follows. Original: CHARGE_OFF
			charge_mode = CHARGE_SLOW;
		}
	}
